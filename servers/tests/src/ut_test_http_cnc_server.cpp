...
TEST(ut_test_http_cnc_server, ctor_dtor_sanity) {
    // GIVEN
    const std::string test_id("test_test_http_cnc_server");
...
TEST(ut_test_http_cnc_server, run_movement_sanity) {
    // GIVEN
    const std::string test_id("test_test_http_cnc_server");
    ...
    TestConnection connection(
        [](const Response& response)-> void {
            std::cout << "test_http_cnc_server sends response, code = " << std::to_string(static_cast<int>(response.code())) << std::endl;
