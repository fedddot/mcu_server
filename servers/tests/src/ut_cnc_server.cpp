...
using Testcnc_server = cnc_server<std::string>;
...
TEST(ut_cnc_server, ctor_dtor_sanity) {
    ...
    // WHEN
    std::unique_ptr<Testcnc_server> instance_ptr(nullptr);

    // THEN
    ASSERT_NO_THROW(instance_ptr = std::unique_ptr<Testcnc_server>(new Testcnc_server(&connection, test_id, &vendor)));
...
TEST(ut_cnc_server, run_is_running_stop_sanity) {
    ...
    // WHEN
    Testcnc_server instance(&connection, test_id, &test_vendor);
