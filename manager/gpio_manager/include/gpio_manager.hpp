...
namespace server {
	...
	class GpioManager: public Resource {
	public:
		...
		GpioManager(const Creator& creator, const Duplicator& duplicator, const Reader& reader, const Writer& writer);
	...
	template <typename Tstored>
	inline GpioManager<Tstored>::GpioManager(const Creator& creator, const Duplicator& duplicator, const Reader& reader, const Writer& writer): m_creator(creator), m_duplicator(duplicator), m_reader(reader), m_writer(writer) {
	...
	template <typename Tstored>
	inline GpioManager<Tstored>::GpioManager(const GpioManager& other): m_creator(other.m_creator), m_duplicator(other.m_duplicator), m_reader(other.m_reader), m_writer(other.m_writer) {
	...
	template <typename Tstored>
	inline Response GpioManager<Tstored>::run_request(const Request& request) const {
		try {
	...
	template <typename Tstored>
	inline Resource *GpioManager<Tstored>::clone() const {
		return new GpioManager(*this);
	}

	template <typename Tstored>
	inline Response GpioManager<Tstored>::run_create(const Request& request) const {
	...
	template <typename Tstored>
	inline Response GpioManager<Tstored>::run_read(const Request& request) const {
	...
	template <typename Tstored>
	inline Response GpioManager<Tstored>::run_update(const Request& request) const {
		if (1UL != request.path().size()) {
			return report_failure(ResponseCode::NOT_FOUND, std::string("path not found"));
		}
		auto id(request.path()[0]);
	...
	template <typename Tstored>
	inline Response GpioManager<Tstored>::run_delete(const Request& request) const {
		if (1UL != request.path().size()) {
			return report_failure(ResponseCode::NOT_FOUND, std::string("path not found"));
		}
		auto id(request.path()[0]);
	...
	template <typename Tstored>
	inline Response GpioManager<Tstored>::report_failure(const ResponseCode& code, const std::string& what) const {
		using Body = typename Response::Body;
		Body failure_body;
