...
using namespace mcu_factory_utl;
...
TEST_F(McuFactoryFixture, ctor_dtor_sanity) {
	...
	// WHEN
	McuFactory<GpioId> *instance_ptr(nullptr);
	McuFactory<GpioId> *instance_ptr_copy(nullptr);
	...
	ASSERT_NO_THROW(
		(
			instance_ptr_copy = new McuFactory<GpioId>(*instance_ptr)
		)
	);
