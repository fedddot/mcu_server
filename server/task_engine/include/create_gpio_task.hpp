...
namespace server {
	...
	inline engine::Data *CreateGpioTask<Tgpio_id>::execute() {
		auto gpio_id = m_gpio_id_parser->parse(access_data());
		auto gpio_dir = m_gpio_dir_parser->parse(access_data());
		access_inventory()->put(gpio_id, m_gpio_creator->create(gpio_id, gpio_dir));
