...
class TestRawWriter: public IpcDataWriter<RawData> {
public:
...
TEST(ut_json_ipc_data_writer, ctor_dtor_sanity) {
	// GIVEN
	const auto raw_data_writer_instance = Instance<IpcDataWriter<RawData>>(
		new TestRawWriter(
			[](const RawData&) {
	...
	// THEN
	ASSERT_NO_THROW(instance = new JsonIpcDataWriter<TestIpcData>(raw_data_writer_instance, transform_ipc_data));
...
TEST(ut_json_ipc_data_writer, write_sanity) {
	...
	auto instance = JsonIpcDataWriter<TestIpcData>(raw_data_writer_instance, transform_ipc_data);
