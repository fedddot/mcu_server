...
TEST(ut_vendor, register_resource_contains_resource_sanity) {
	...
	const TestResource test_resource_1(test_resource_id_1, stub);
	const TestResource test_resource_2(test_resource_id_2, stub);
	
	// WHEN
	Vendor instance(test_vendor_id);

	// THEN
	ASSERT_FALSE(instance.contains_resource(test_resource_id_1));
	ASSERT_NO_THROW(instance.register_resource(test_resource_1));
	ASSERT_TRUE(instance.contains_resource(test_resource_id_1));

	ASSERT_FALSE(instance.contains_resource(test_resource_id_2));
	ASSERT_NO_THROW(instance.register_resource(test_resource_2));
...
TEST(ut_vendor, run_request_sanity) {
	...
	// WHEN
	Vendor instance(test_vendor_id);
	instance.register_resource(test_resource_1);
	instance.register_resource(test_resource_2);
